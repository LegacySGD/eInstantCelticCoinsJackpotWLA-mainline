			...
			<lxslt:component prefix="my-ext" functions="formatJson,retrievePrizeTable,getType">
				...
				<![CDATA[
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNamesDesc)
					{
						...
						if (bonusTotal == 5) 
						{
							bonusPrizeValue = getTranslationByName("jackpot", translations);
						}
						r.push('<td>');
						...
						if (bonusHorseshoe)
						{
							var bonusHorseshoeData = getBonusValue(scenario, 2);
							r.push('<table border="0" cellpadding="2" cellspacing="1" class="gameDetailsTable">');
							r.push('<tr>');
 							r.push('<td class="tablehead">');
 							r.push(getTranslationByName("bonusHorseshoe", translations));
 							r.push('</td>');
							r.push('<td class="tablehead">');
 							r.push(getTranslationByName("bonusPrize", translations));
	 						r.push('</td>');
							...
							r.push('<tr>');
	 						...
	 						r.push('</td>');
	 						r.push('<td>');
							if (bonusHorseshoeData == "VJ")
							{
								r.push(getTranslationByName("jackpot", translations));
							}
							else
							{
								r.push(convertedPrizeValues[getPrizeNameIndex(prizeNames, bonusHorseshoeData)]);
							}
	 						r.push('</td>');
							r.push('</tr>');
							r.push('</table>');
						...
						if (bonusCoin)
						{
							var bonusCoinData = getBonusValue(scenario, 3);
							var bonusCoinWins = bonusCoinData.split(",");
							r.push('<table border="0" cellpadding="2" cellspacing="1" class="gameDetailsTable">');
							r.push('<tr>');
 							r.push('<td class="tablehead">');
 							r.push(getTranslationByName("bonusCoin", translations));
 							r.push('</td>');
							r.push('<td class="tablehead">');
 							r.push(getTranslationByName("bonusPrize", translations));
	 						r.push('</td>');
							r.push('</tr>');
							r.push('<tr>');
							if (bonusCoinWins[bonusCoinWins.length -1] == "PJ")
							{
		 						r.push('<td>');
		 						r.push('</td>');
		 						r.push('<td>');
							...
							{
								for (var i = 0; i < bonusCoinWins.length -1; ++i)
								{
									r.push('<tr>');
			 						r.push('<td>');
			 						r.push('</td>');
		 							r.push('<td>');
									r.push(convertedPrizeValues[getPrizeNameIndex(prizeNames, bonusCoinWins[i])]);
		 							r.push('</td>');
									r.push('</tr>');
								}
								r.push('<tr>');
					...
					function getBonusValue(scenario, index)
					{
						var numsData = scenario.split("|")[index];
						return numsData;
					}

					// Input: "23,9,31|8:E0,35:E0,4:D1,13:D0,37:G0,..."
					// Output: ["8", "35", "4", "13", ...] , ["E", "E", "D", "G", ...] or ["0", "0", "1", "0", "0",]
					...
					function translateOutcomeNumber(outcomeNum, translations)
					{
						if (outcomeNum == "T")
						{
							return getTranslationByName("bonusHorseshoeTrigger", translations);
						}
						else if (outcomeNum == "U")
						{
							return getTranslationByName("bonusCoinTrigger", translations);
						}
